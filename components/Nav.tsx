...
export default function Nav({
  ...
  copy: Dictionary["nav"];
  name: string;
  /* Cal.com booking link. Empty falls back to the contact section. */
  cal?: string;
    ...
    <header className={`fixed inset-x-0 top-0 z-50 transition-colors duration-300 ${scrolled ? "bg-background/90 border-b border-border backdrop-blur-md" : "border-b border-transparent"}`}>
      <div className="mx-auto flex h-14 max-w-[1920px] items-center justify-between px-5 sm:h-16 md:px-10 lg:h-20 lg:px-[60px]">
        ...
        <nav className="hidden items-center gap-8 lg:flex">
          ...
          <Link href={cal || `${base}#contact`} className="btn rounded-md border border-accent/40 bg-accent/5 px-4 py-2 text-[15px] font-[550] text-nav">{copy.cta}</Link>
        </nav>
        ...
        <nav className="border-t border-border bg-background px-5 py-6 lg:hidden">
          <div className="flex flex-col gap-4">
            ...
            <div className="mt-2 flex items-center justify-between border-t border-border pt-4">
              <LangSwitch current={lang} rest={rest} />
              <Link href={cal || `${base}#contact`} className="rounded-md border border-accent/40 bg-accent/5 px-4 py-2 text-[15px] font-[550] text-nav">{copy.cta}</Link>
            </div>
          </div>
        </nav>
      )}
